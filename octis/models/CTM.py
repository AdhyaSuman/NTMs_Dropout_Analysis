...
class CTM(AbstractModel):
    ...
    def __init__(self, num_topics=10, model_type='prodLDA', activation='softplus',
                 theta_dropout=0.2, enc_dropout=0.2, learn_priors=True, batch_size=64, lr=2e-3, momentum=0.99,
        ...
        self.hyperparameters['theta_dropout'] = theta_dropout
        self.hyperparameters['enc_dropout'] = enc_dropout
    ...
    def train_model(self, dataset, hyperparameters=None, top_words=10):
        ...
        if self.use_partitions and self.use_validation:
            ...
            x_train, x_test, x_valid, input_size = self.preprocess(
                ...
                bert_train_path=self.hyperparameters['bert_path'] + self.hyperparameters["bert_model"] + "_train.pkl",
                bert_test_path=self.hyperparameters['bert_path'] + self.hyperparameters["bert_model"] + "_test.pkl",
                bert_val_path=self.hyperparameters['bert_path'] + self.hyperparameters["bert_model"] + "_val.pkl",
            ...
            self.model = ctm.CTM(input_size=input_size, bert_input_size=x_train.X_bert.shape[1], model_type='prodLDA',
                                 ...
                                 theta_dropout=self.hyperparameters['theta_dropout'],
        ...
        elif self.use_partitions and self.use_validation==False:
            ...
            self.model = ctm.CTM(input_size=input_size, bert_input_size=x_train.X_bert.shape[1], model_type='prodLDA',
                                 ...
                                 reduce_on_plateau=self.hyperparameters['reduce_on_plateau'],
                                 topic_prior_variance=self.hyperparameters["prior_variance"])
            self.model.fit(x_train, verbose=False)
            result = self.inference(x_test)
            return result
            ...
            x_train, input_size = self.preprocess(
                self.vocab, train=data_corpus, bert_train_path=self.hyperparameters['bert_path'] + self.hyperparameters["bert_model"] + "_train.pkl",
                bert_model=self.hyperparameters["bert_model"])
