...
class DecoderNetwork(nn.Module):
    ...
    def __init__(self, input_size, bert_size, infnet, n_components=10, model_type='prodLDA',
                 hidden_sizes=(100,100), activation='softplus', theta_dropout=0.2, enc_dropout=0.2,
        ...
        assert theta_dropout >= 0, "dropout must be >= 0."
        assert enc_dropout >= 0, "dropout must be >= 0."
        ...
        print('-'*35)
        print('theta_dropout={}, enc_dropout={}'.format(theta_dropout, enc_dropout))
        print('-'*35)

        if infnet == "zeroshot":
            self.inf_net = ContextualInferenceNetwork(
                input_size, bert_size, n_components, hidden_sizes, activation, dropout=enc_dropout)
        ...
        # dropout on theta
        self.drop_theta = nn.Dropout(p=self.theta_dropout)
